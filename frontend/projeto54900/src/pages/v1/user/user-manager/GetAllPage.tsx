// Lista de usuarios (view_user_manager) — index do modulo user-manager.
//
// Fork de FormConstructorListPage.tsx (mesmo padrao de producao do motor
// "Construtor de Listas" — list_manager/list_columns/list_actions -> ver
// README_list_constructor.md / README_render_via_list_constructor.md):
// colunas e acoes NAO estao fixas no codigo, vem da definicao gravada no
// banco (list_manager de slug 'user-manager', table_name = view_user_manager).
// Acoes executam de verdade (Link real / chamada HTTP real).
//
// Diferenca pro fork original: acrescenta caixa de busca (useDebounce,
// 400ms) chamando manager.apiSearchEndpoint com q=termo — 1a tela deste
// projeto a exercitar esse campo do list_manager de verdade.
//
// Espelha o padrao REST do backend:
//   /v1/user-manager              -> esta lista (get-all de view_user_manager)
//   /v1/user-manager/create       -> CreatePage
//   /v1/user-manager/:id          -> GetPage (detalhe)
//   /v1/user-manager/update/:id   -> UpdatePage
...
export default function GetAllPage() {
  ...
  // 1) Carrega a definicao (list_manager + list_columns + list_actions) do slug user-manager.
  ...
  // 2) Busca os dados reais (paginacao/sort da URL) — api_get_endpoint normalmente,
  //    api_search_endpoint (com q=termo) quando a busca esta preenchida.
