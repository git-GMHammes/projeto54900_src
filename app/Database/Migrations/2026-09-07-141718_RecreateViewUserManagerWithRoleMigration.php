...
/**
 * Recria a view view_user_manager incluindo o perfil do usuario.
 *
 * Acrescenta ao SELECT original:
 *   um.user_role_id AS um_user_role_id   (FK crua)
 *   ur.slug         AS ur_role_slug
 *   ur.name         AS ur_role_name
 * via LEFT JOIN user_roles ur (ur.id = um.user_role_id AND ur.deleted_at IS NULL).
 *
 * Prefixos:
 *   um_ = user_manager   uc_ = user_profiles   ur_ = user_roles
 *
 * Depende de 2026-09-07-141717_AddUserRoleIdToUserManagerMigration (coluna
 * user_manager.user_role_id). down(): recria a view sem o perfil.
 */
class RecreateViewUserManagerWithRoleMigration extends Migration
{
    public function up()
    {
