-- -----------------------------------------------------------------------------
-- Views do banco codeigniter54900_db — dump formatado em 2026-09-25.
...
-- Diferença em relação a 202609241714_create_view.sql: as quatro views
-- anteriores seguem idênticas; entra a quinta, view_user_directory (diretório
-- mínimo de usuários, usado no campo de escolha de convidado do calendário).
-- -----------------------------------------------------------------------------
SET NAMES utf8mb4;
-- -----------------------------------------------------------------------------
-- view_calendar_manager
-- calendar_manager (cm) + calendar_events (ce), junção à esquerda — só 2
-- níveis, de propósito: os 4 ramos-filhos de calendar_events (attendees,
-- reminders, attachments, extended_properties) ficam de fora, evitando
-- produto cartesiano. Um calendário sem eventos ainda aparece, com todo
-- ce_* vazio.
-- -----------------------------------------------------------------------------
...
-- achata os 4 níveis, 1 linha por campo. `id` = form_fields.id (vazio num
-- ramo sem campos). Cada junção descarta os registros com deleted_at
-- preenchido do próprio nível.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- view_upload_manager
-- Projeção direta de uploads (sem junção) — colunas prefixadas up_.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- view_user_directory
-- user_manager (um) + user_profiles (uc), junção à esquerda — diretório
-- mínimo de usuários (identificação e contato). Fica de fora o que a
-- view_user_manager expõe de sensível: status, perfil de acesso, último
-- acesso, documento e endereço. Usuário sem perfil ainda aparece, com
-- uc_name e uc_email vazios.
-- Consumida por /api/v1/user-directory-view (qualquer usuário logado).
-- No banco de hoje esta view nasceu com conjunto de caracteres latin1
-- (cliente conectado em latin1, provável descuido); aqui ela é recriada em
-- utf8mb4, como as outras quatro — sem efeito nos resultados, pois a
-- definição não tem texto literal, só colunas das tabelas de origem.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- view_user_manager
-- user_manager (um) + user_profiles (uc) + user_roles (ur), junção à
-- esquerda.
-- -----------------------------------------------------------------------------
