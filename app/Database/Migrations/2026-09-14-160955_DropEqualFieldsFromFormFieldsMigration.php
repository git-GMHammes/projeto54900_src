...
/**
 * Remove a coluna equal_fields de form_fields.
 *
 * Motivo: redundante com double_field para o unico tipo que usa as duas
 * (senha) — um campo de confirmacao ("Confirmar Senha") sem exigir igualdade
 * nao faz sentido de produto. double_field sozinho passa a sempre exigir
 * igualdade entre os 2 campos (ver SenhaField em
 * src/frontend/projeto54900/src/components/ui/FormGrid/senha/index.tsx).
 *
 * up():   dropColumn(equal_fields) + recria view_form_manager sem fc_equal_fields.
 * down(): readiciona a coluna (TINYINT(1) DEFAULT 0) e a view com ela.
 */
class DropEqualFieldsFromFormFieldsMigration extends Migration
{
    public function up()
    {
        $this->forge->dropColumn('form_fields', 'equal_fields');
    ...
    /**
     * Recria view_form_manager. Com $withEqualFields = true inclui a coluna
     * fc_equal_fields (estado anterior a esta migration).
     */
    private function recreateView(bool $withEqualFields): void
    {
        $this->db->query('DROP VIEW IF EXISTS `view_form_manager`');

        $equalFields = $withEqualFields
        ...
        $template = <<<'SQL'
            ...
            SELECT
                ...
                %sfc.with_seconds         AS fc_with_seconds,
        ...
        $this->db->query(\sprintf($template, $equalFields));
    }
}
